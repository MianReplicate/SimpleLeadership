...
namespace SimpleLeadership
{
    ...
    public static class PawnGenerator_GeneratePawn_Patch
    {
        ...
        {
            if (__result != null && !request.ForceDead && request.Context == PawnGenerationContext.NonPlayer && request.Tile.Valid)
            {
                Settlement settlement = Find.WorldObjects.SettlementAt(request.Tile);
                if (settlement != null && settlement.Tile.Valid && settlement.IsInPowerEvent(PowerEventDefOf.SL_Famine))
                {
